...
root -l -q "squadron_cuts_delphes.C(\"delphes_events.root\",\"runname_num\")"

How to compile the script:
First add ~/MG5_aMC_v2_1_2/ExRootAnalysis/lib to your LDLIBRARYPATH
g++ `root-config --cflags` -I ~/MG5_aMC_v2_1_2/ExRootAnalysis/ -I ~/MG5_aMC_v2_1_2/Delphes/ squadron_cuts_delphes.C `root-config --glibs` -L ~/MG5_aMC_v2_1_2/ExRootAnalysis/lib -lExRootAnalysis -L ~/MG5_aMC_v2_1_2/Delphes -lDelphes -o squadron_cuts_delphes -Ofast
How to run the compiled script:
./squadron_cuts_delphes delphes_events.root runname_num
*/
...
void squadron_cuts_delphes(const char *rootFile, const char *runname_num);

int main (int argc, char *argv[]) {
  if (argc != 3) {cout << "arg1: rootFile, arg2: runname_num\n"; return 1;}
  squadron_cuts_delphes(argv[1], argv[2]);
  return 0;
}
...
void squadron_cuts_delphes(const char *rootFile, const char *runname_num) {
   ...
   TClonesArray *bchJet = treeReader->UseBranch("Jet");
   TClonesArray *bchElectron = treeReader->UseBranch("Electron");
   TClonesArray *bchMuon = treeReader->UseBranch("Muon");
   TClonesArray *bchMET = treeReader->UseBranch("MissingET");

   ofstream resultsfile;// All-in-one output file
   stringstream sseff;
   sseff << runname_num << "_results.txt";
   resultsfile.open(sseff.str().c_str());
   resultsfile << "CutName\txsec no cuts\txsec w/ cuts\t#Eve nc\t#Eve wc\tEfficiency\t#EventsNJets" << endl;
//TODO Need to modify gg_xsecs_out.dat code in cycle_squadron_delphes.
...
stringstream ssxsec;
ssxsec << "xsec";
ifstream xsecfile(ssxsec.str().c_str());
if (xsecfile.fail()) {cout << "Error! " << ssxsec.str() << " does not exist." << endl; return;}
Double_t crossx;
string ignore;
xsecfile >> ignore >> ignore >> ignore >> crossx;//ignore the leading text
xsecfile.close();
cout << "Cross section " << crossx << endl;
crossx *= 1000.0;//convert cross section to femtobarns

int numcuts;
string cutname;
Double_t CUTJETETA, CUTJETPTHI, CUTJETPTLO, CUTMETMIN, DELTARMIN, DELTAPHIMINJ3MET, NPTCUTJETS, NJETSMIN, HTMIN, HT5MIN, TAUTAGMIN;
...
if (paramfile.fail()) {cout << "Error! ../../cut_parameters does not exist." << endl; return;}
   ...
   Int_t numEventsPassCuts = 0,numEventsWithNJets[25];
   Double_t HT,HT5;
   ...
   for(Int_t ijet=0; ijet<25; ijet++){ numEventsWithNJets[ijet] = 0; }

   ofstream eventfile;// Output file which lists the number of every event which passes the cuts
   stringstream ssevent;
   ssevent << runname_num << "_" << cutname << "_event_numbers.txt";
   eventfile.open(ssevent.str().c_str());
   eventfile << "The following events passed all cuts:\n";

   // Loop over all events
   Long64_t numberOfEvents = treeReader->GetEntries();
   for(Long64_t eventNum = 0; eventNum < numberOfEvents; ++eventNum) {
     treeReader->ReadEntry(eventNum);

     Int_t numJets = bchJet->GetEntriesFast();
     Int_t numElectrons = bchElectron->GetEntriesFast();
     Int_t numMuons = bchMuon->GetEntriesFast();
...
// enough jets and ignore / "veto" all events with charged leptons of any sort
     if((numJets >= NJETSMIN) && (numElectrons + numMuons == 0)) {
...
// and set initial values for H_T and H_T5 before the jet p_T are added to the sum
       ...
       MissingET* met = (MissingET*) bchMET->At(0);
       HT = met->MET;
       HT5 = 0.0;
       Bool_t jetsok = true;
...
// Note: Delphes already sorts numerical values, so no need to manually sort jets by PT
       for (Int_t ijet=0; ijet<numJets; ijet++){
         Jet* jet1 = (Jet*) bchJet->At(ijet);
         if (jet1->TauTag) { numTauTag++; }         // Tau Tagging
         TLorentzVector vjet1;
         vjet1.SetPtEtaPhiM(jet1->PT, jet1->Eta, jet1->Phi, jet1->Mass);
         HT += jet1->PT;                            // add jet p_T to H_T
         if (ijet < 5) { HT5 += jet1->PT; }         // add jet p_T to H_T5
         if (jet1->PT >= CUTJETPTLO) { numJetsPassPtCut++; }  // count the jet
         for (Int_t jjet=ijet+1; jjet<numJets; jjet++){
           Jet* jet2 = (Jet*) bchJet->At(jjet);
           TLorentzVector vjet2, vmet;
           vjet2.SetPtEtaPhiM(jet2->PT, jet2->Eta, jet2->Phi, jet2->Mass);
           vmet.SetPtEtaPhiM(met->MET,0,met->Phi,0);// use met->Eta?
           Double_t Pi = 3.14159265; Double_t Degree = Pi / 180.0;
           if (vjet1.DeltaR(vjet2) < DELTARMIN ||                                      // apply Delta R_jj criterion
              (jet1->PT<CUTJETPTHI && ijet<NPTCUTJETS) ||                              // apply p_T criterion
...
//Now we can apply the final cuts and record the event
       if (jetsok && // verifies that jet isolation cuts are satisfied
           ...
           met->MET >= CUTMETMIN &&                 // apply MET cut
         ...
         Histos[1]->Fill(met->MET);                 // Missing Energy
...
// list event numbers of events passing all cuts in the output text file	      
   ...
   stringstream ssroot;
   ssroot << runname_num << "_" << cutname << "_out.root";
...
resultsfile.close();
paramfile.close();
} // end of squadron_cuts_delphes function
